...
shinyUI(
  navbarPage(theme = shinytheme("yeti"),
    ...
    tabPanel(
      ...
      p("Since the 1990s, there has been a precipitous increase in the frequency and severity of water conflicts around the world. 
      ...
      fluidPage(
        fluidRow(style = 'padding:30px;',
                 ...
                 column(5,
                   ...
                   p("As the graph at left shows, there has been a general upward trend in water conflict over time, with an especially significant increase beginning in the 1990s. 
        ...
        fluidRow(column(4,
          ...
          p("As can be seen in the map at right, water conflict is a ubiquitous phenomenon.
    ...
    tabPanel(
      ...
      p("This project's dataset is the first publicly-available data source to provide comprehensive statistics at a basin level for all of the world's major River Basins. 
        ...
        tags$ul(
          tags$li("Basin GDP is country-level GDP for each year, either summed or averaged across each river basin (source: World Bank)"),
          tags$li("Basin population is country-level population for each year, either summed or averaged across each river basin (source: United Nations"),
          tags$li("Trade as a percentage of GDP is given at a country level for each year, averaged across each river basin (source: World Bank)"),
          tags$li("Rate of water consumption is the amount of water each basin country consumes, divided by the country's total available water resources. This metric is averaged across each river basin (source: World Bank)"),
          tags$li("Agricultural land is the total area of agricultural land per country for each year (in km^2), either summed or averaged (source: World Bank)"),
          tags$li("Percent of population affected by drought is the percentage of each country's population that experienced a drought event over the 60 year period, averaged across each river basin (source: World Bank)"),
          tags$li("The Democratization index score is the democracy score (from 1 to 10, where 10 is most democratic) assigned to each country in 2015, averaged across each river basin (source: EIU Democratization Index)"),
          tags$li("All of the above variables can be logged() as well, which can improve the quality of regression.")),
        p("Having so many statistics at the basin (rather than country) level provides a rich opportunity to test various hypotheses about water conflict. Use the dashboard below to construct a model. 
      ...
      mainPanel(
        ...
        p("Now, let's take the model above and predict the number of water conflict events over the next 50 years for a a hypothetical river basin with the inputs given at left.
  ...
  tabPanel("Key Issues",
           sidebarLayout(
             ...
             # Show Word Cloud
             mainPanel(
               p("Event frequency can only tell us so much. 
  ...
  tabPanel("Case Studies",
           h2("Case Studies"),
           p("Aggregate data are useful, but it is also important to examine variation and nuances between individual river basins.
           ...
           fluidRow(
             column(
               ...
               selectInput(
                 "river_basin",
                 "River Basin:",
                 choices = c("Aral Sea", "Cestos", "Helmand", "Indus", "Akpa", "Alsek", "Amacuro", "Amur", "An Nahr Al Kabir", "Asi/Orontes", "Artibonite", "Astara Chay", "Atrak", "Aviles", "Awash", "Aysen", "Baker", "Bann", "Bidasoa", "Benito/Ntem", "Bia", "Beilun", "Belize", "Bangau", "Baraka", "Barima", "Barta", "Buzi", "Candelaria", "Changuinola", "Carmen Silva/Chico", "Chira", "Choluteca", "Chuy", "Chilkat", "Columbia", "Chiloango", "Cancoso/Lauca", "Congo/Zaire", "Coco/Segovia", "Comau", "Cross", "Coruh", "Castletown", "Coatan Achute", "Catatumbo", "Cullen", "Cavally", "Daoura", "Dnieper", "Dniester", "Don", "Dragonja", "Dra", "Dasht", "Daugava", "Douro/Duero", "Ebro", "Elancik", "Erne", "Cuvelai/Etosha", "Fane", "Flurry", "Fenney", "Foyle", "Fraser", "Firth", "Gash", "Grijalva", "Glama", "Golok", "Goascoran", "Garonne", "Great Scarcies", "Guadiana", "Guir", "Gauja", "Han", "Hari/Harirud", "Hamun-i-Mashkel/Rakshan", "Hondo", "Bei Jiang/Hsi", "Incomati", "Ili/Kunes He", "Isonzo", "Jayapura", "Jacobs", "Jordan", "Juba-Shibeli", "Jurado", "Kaladan", "Kemi", "Kogilnik", "Komoe", "Karnaphuli", "Kowl E Namaksar", "Krka", "Klaralven", "Kunene", "Kura-Araks", "Lava/Pregel", "Lotagipi Swamp", "Lima", "Lake Chad", "Lake Fagnano", "Lake Natron", "Lake Titicaca-Poopo System", "Lake Turkana", "Lake Ubsa-Nur", "Lielupe", "Lough Melvin", "Lempa", "Limpopo", "Lagoon Dos Patos-Lagoon Mirim", "Loes", "Loffa", "Little Scarcies", "Mana-Morro", "Maro", "Massacre", "Mbe", "Medjerda", "Mekong", "Muhuri (aka Little Feni)", "Mino", "Mira", "Mius", "Moa", "Moho", "Mono", "Motaqua", "Murgab", "Maritsa", "Naatamo", "Negro", "Nelson-Saskatchewan", "Nahr El Kebir", "Neman", "Neretva", "Narva", "Nestos", "Nyanga", "Oued Bon Naima", "Oder/Odra", "Ogooue", "Okavango", "Olanga", "Oral/Ural", "Orange", "Oueme", "Oulu", "Oiapoque/Oyupock", "Pangani", "Paz", "Pedernales", "Pakchan", "Palena", "Pandaruan", "Po", "Prohladnaja", "Parnu", "Pascua", "Psou", "Pasvik", "Patia", "Puelo", "Pu Lun T'o", "Red/Song Hong", "Rezvaya", "Rio Grande (North America)", "Rio Grande (South America)", "Rhone", "Roia", "Sabi", "Salaca", "Samur", "Sanaga", "Sassandra", "Sebuku", "Seine", "Senegal", "Seno Union/Serrano", "Sepik", "Shu/Chu", "Sixaola", "St. John (North America)", "San Juan", "Skagit", "St. Lawrence", "San Martin", "Sembakung", "St. Paul", "Sarata", "Sarstun", "Stikine", "Struma", "Suchiate", "Sujfun", "Tafna", "Tagus/Tejo", "Taku", "Talas", "Tami", "Tana", "Tano", "Temash", "Terek", "Tijuana", "Tjeroaka-Wanggoe", "Torne/Tornealven", "Tarim", "Tuloma", "Tumbes", "Umba", "Utamboni", "Vanimo-Green", "Valdivia", "Venta", "Vijose", "Velaka", "Volga", "Vardar", "Vistula/Wista", "Vuoksa", "Whiting", "Wiedau", "Yalu", "Yaqui", "Yelcho", "Jenisej/Yenisey", "Yser", "Yukon", "Zapaleri", "Essequibo", "Amazon", "Lake Prespa", "Elbe", "Zambezi", "Zarumilla", "Niger", "Gambia", "Pungwe", "Mississippi", "Gallegos/Chico", "La Plata", "Orinoco", "Digul", "Maputo", "Nile", "Drin", "Adige", "Annole", "Bahr at Tubat", "Oued Bou Namoussa", "Caetani", "Cetina", "Connecticut", "Copper", "Galana", "Gruzskiy Yelanchik", "Indalsalven", "Laguna Filaret", "Lak Dera", "Lake Azuei", "Lake Cayo", "Lake Chilwa", "Lake Enriquillo", "Lake Rukwa", "Lake Sarygamesh", "Lucia", "Narynka", "Nidelva", "Peschanaya", "Poldnevaya", "Rach Giang Thanh", "Santa Clara", "Song Tien Yen", "Berbyelva", "Umbeluzi", "Unuk", "Vecht", "Vefsna", "Angerman", "Colorado", "Connecticut", "Danube", "Fly", "Ganges-Brahmaputra-Meghna", "Geba-Corubal", "Mataje", "Rann of Kutch", "Rhine-Meuse", "Ruvuma", "Nha Be-Saigon-Song Vam Co Dong", "St. Croix", "Schelde", "St. John (Africa)", "Tumen", "Volta", "Corantijn/Courantyne", "Har Us Nur", "Maroni", "Ob", "Sulak", "Alakol", "Ca/Song Lam", "Irrawaddy", "Ma", "Salween", "Tigris-Euphrates/Shatt al Arab", "Naaf River")),
