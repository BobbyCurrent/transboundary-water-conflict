...
# type, and recoded appropriate values as NA. Once it becomes clearer which data is relevant to my
# final product, I will be able to further narrow down columns and perform even
# more rigorous data cleaning.
...
# and "issue_type2") provided the same data in a format that was much easier to
# parse.
...
# The "treaties" data is also already in tidy format. It contains many, many
# columns (114), and it is unclear precisely which will be most relevant to my
# analysis. Thus, I refrained from recoding any of their values until I have a
# better idea where I want to take the final project. For now, I replaced the
# appropriate values with NA and selected the relevant columns.
...
# Now, I join my data together into one table. Performing all these join
...
# tables. I suppose it is also possible that I could join my data into 2 or 3
# separate tables, but I am interested in analyzing and plotting these data as a
# unit, so ideally they will be in one table.
...
# bccode2 transfers as its own column, so it should be okay that I am only merging based on bccode 1
